...
add_image_size( 'head_img', 69, 54 );
add_image_size( 'highlight_img', 109, 60 );
...
if ( has_post_thumbnail( get_the_ID() ) ) {
						$img_obj = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), array( 109, 60 ) );
						$img     = $img_obj[0];
} else {
	$img = get_bloginfo( 'template_directory' ) . '/timthumb.php?src=' . get_post_meta( $post->ID, 'image_value', true ) . '&amp;w=109&amp;h=60&amp;zc=1';
}
?>

							<img src="<?php echo $img; ?>" alt="<?php the_title(); ?>" width="109px" height="60px" />
...
function register_my_menus() {
	register_nav_menus(
		array( 'header-menu' => __( 'Header Menu', 'Magazeen_Theme' ) )
	);
}
